        ...
        if self.missing_num or self.missing_categ:
            ...
            if self.count_missing != 0:
                ...
                if self.missing_num:  # numeric data
                    logger.info(
                        'Started handling of NUMERICAL missing values... Method: "{}"',
                    ...
                    # delete missing values
                        ...
                        logger.debug(
                            "Deletion of {} NUMERIC missing value(s) succeeded",
                            self.count_missing - df.isna().sum().sum(),
                        )

                if self.missing_categ:  # categorical data
                    logger.info(
                        'Started handling of CATEGORICAL missing values... Method: "{}"',
            ...
            logger.info(
                "Completed handling of missing values in {} seconds",
                round(end - start, 6),
            )
        else:
                ...
                if feature in cols_num:
                    if df[feature].isna().sum().sum() != 0:
                        try:
                            ...
                            if counter != 0:
                                logger.debug(
                                    '{} imputation of {} value(s) succeeded for feature "{}"',
                                    str(self.missing_num).upper(),
                                    counter,
                                    feature,
                                )
                ...
                if feature not in cols_num:
                    if df[feature].isna().sum() != 0:
                        try:
                            ...
                            mappings = {
                                k: i
                                for i, k in enumerate(df[feature].dropna().unique(), 0)
                            }
                            ...
                            counter = sum(
                                1
                                for i, j in zip(
                                    list(df_imputed[feature]), list(df[feature])
                                )
                                if i != j
                            )
            ...
            try:
                ...
                if len(test_df.index) != 0:
                    ...
                    if (df[feature].fillna(-9999) % 1 == 0).all():
                        ...
                        df[feature].update(test_df[feature])
                    logger.debug(
                        'LINREG imputation of {} value(s) succeeded for feature "{}"',
                        len(pred),
                        feature,
                    )
            except:
    ...
    def _log_regression_impute(self, df, model):
        ...
        for feature in df.columns:
            if feature not in cols_num:
                # create label mapping for categorical feature values
                mappings = {
                    k: i for i, k in enumerate(df[feature])
                }  # .dropna().unique(), 0)}
                mapping[feature] = mappings
                df[feature] = df[feature].map(mapping[feature])
                ...
                try:
                    ...
                    if len(test_df.index) != 0:
                        ...
                        if (df[feature].fillna(-9999) % 1 == 0).all():
                            ...
                            df[feature].update(test_df[feature])
                        logger.debug(
                            'LOGREG imputation of {} value(s) succeeded for feature "{}"',
                            len(pred),
                            feature,
                        )
                except:
...
class Outliers:
    ...
    def _winsorization(self, df):
        ...
        for feature in cols_num:
            ...
            if str(df[feature].dtype).lower() == "int64":
...
class Adjust:
    def convert_datetime(self, df):
        ...
        if self.extract_datetime:
            logger.info(
                "Started conversion of DATETIME features... Granularity: {}",
    ...
    def round_values(self, df, input_data):
        ...
        if (
            self.duplicates
            or self.missing_num
            or self.missing_categ
            or self.outliers
            or self.encode_categ
            or self.extract_datetime
        ):
            ...
            logger.info(
                "Completed feature type conversion for {} feature(s) in {} seconds",
                counter,
                round(end - start, 6),
            )
        else:
...
class EncodeCateg:
    ...
    def _to_onehot(self, df, feature, limit=10):
        ...
        if one_hot.shape[1] > limit:
            logger.warning(
                'ONEHOT encoding for feature "{}" creates {} new features. Consider LABEL encoding instead.',
                feature,
                one_hot.shape[1],
            )
        # join the encoded df
